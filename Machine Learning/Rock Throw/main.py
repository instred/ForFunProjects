...
def gameOver() -> None:
    ...
    while game_over:
        ...
        gameOverSurf = gameOverFont.render('Game Over, press R to respawn', True, BLACK)
...
def gameLoop() -> None:
    ...
    while game_run:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if main_rock.isThrowing:
                    main_rock.throw(power, angle_degrees)
        ...
        surface = pygame.draw.line(game_window, BLACK, (0, LINE_Y), (WINDOW_WIDTH, LINE_Y))
        main_player.show()
        main_goal.show()
        main_rock.show()
        if game_start:
            main_rock.update(main_goal)
            main_player.setDistance(main_rock.bounces)
            if main_rock.collide(main_goal):

                gameOver()
                main_player.score = main_rock.bounces
                game_start = False

        distance_text = font.render(f"Score: {main_player.score:.2f}", True, BLACK)
        ...
        if drawing and start_pos is not None:
            ...
            shot_text = font.render(f"Shot Power: {power:.2f}%", True, BLACK)
